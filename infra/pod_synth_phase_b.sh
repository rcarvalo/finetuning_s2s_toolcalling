...
# importable for the CLIs without registering anything.
export PYTHONPATH=/repo/python
python3 -m pip install -q numpy pydantic pydantic-settings pyyaml huggingface-hub 2>&1 | tail -1
# torchaudio pinned to the image's torch (same minor) with --no-deps: a
...
# path must start its own. VLLM_PLUGINS="" belts-and-braces the plugin issue.
VLLM_PLUGINS="" nohup vllm serve mistralai/Voxtral-4B-TTS-2603 --omni \
