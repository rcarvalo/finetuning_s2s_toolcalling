"""Build the v4 training set: Phase A (single-turn) + Phase B (conversational).
...
def fetch_phase_b() -> list[dict[str, Any]]:
    ...
    jsonl = hf_hub_download(REPO, "phase_b/train_v4.jsonl", repo_type="dataset")
    tarballs = [
        ...
        # Les 297 aveux d'échec de v4 : leur ancienne réponse a été réécrite,
        # donc leur audio aussi (cinq clips partagés, voix Aiden identique).
        hf_hub_download(REPO, "phase_b/miss_audio.tar.gz", repo_type="dataset"),
    ]
...
def main() -> None:
    ...
    for split, dataset_dir in (("train", "datasets/tc_en_v4_train"), ("val", "datasets/tc_en_v4_val")):
    ...
    print("PREPARE_V4_DONE", flush=True)
