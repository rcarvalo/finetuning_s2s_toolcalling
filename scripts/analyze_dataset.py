...
et — si l'audio est présent — durées / silence / fichiers manquants / voix.
    ...
    python scripts/analyze_dataset.py --dialogues data/tc_en_train.audio.jsonl \
...
def load_rows(path: str | Path) -> list[dict[str, Any]]:
    return [json.loads(s) for s in (l.strip() for l in Path(path).open(encoding="utf-8")) if s]
...
def text_quality(rows: list[dict]) -> dict[str, Any]:
    words = [len(_utt(r).split()) for r in rows]
    ...
    for tool, key in (("web_search", "query"), ("db_query", "question")):
        ...
        if vals:
            out["args"][tool] = {
                ...
                "words": _stat([len(v.split()) for v in vals]),
...
def voices(rows: list[dict]) -> dict[str, int]:
    return dict(Counter(t.get("voice") for r in rows for t in r["turns"]
                        if t.get("role") == "user" and t.get("voice")))


def audio_quality(rows: list[dict], audio_root: str | Path, *, rms_sample: int = 150,
                  seed: int = 0) -> dict[str, Any]:
    import soundfile as sf

    root = Path(audio_root)
    paths = [(r["id"], root / t["audio"]) for r in rows for t in r["turns"]
             if t.get("role") == "user" and t.get("audio")]
    missing = [str(p) for _, p in paths if not p.exists()]
    durations, rates = [], Counter()
    for _, p in paths:
        if p.exists():
            info = sf.info(str(p))
...
def _stat(xs: list[float]) -> dict[str, float]:
    if not xs:
        return {"n": 0}
    return {"n": len(xs), "min": round(min(xs), 2), "p50": round(st.median(xs), 2),
            "max": round(max(xs), 2), "mean": round(st.mean(xs), 2)}
...
def main() -> None:
    ...
    issues = flags(rep)
    print("\n" + ("✅ dataset sain — aucun drapeau" if not issues else
                  "⚠️ drapeaux :\n - " + "\n - ".join(issues)))
