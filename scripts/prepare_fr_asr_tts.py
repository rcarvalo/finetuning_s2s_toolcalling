...
``python -m s2s_toolcalling.data.preprocess_sft --assistant-audio-mode sequential``.
...
Usage :
    python scripts/prepare_fr_asr_tts.py --manifest cv_fr.jsonl \\
...
def main() -> None:
    ...
    parser.add_argument("--tts-ratio", type=float, default=0.5, help="part des échantillons convertis en TTS (si les deux tâches)")
    ...
    with Path(args.manifest).open(encoding="utf-8") as fin, Path(args.output).open("w", encoding="utf-8") as fout:
        for i, line in enumerate(fin):
            ...
            if len(args.tasks) == 2:
                task = "tts" if rng.random() < args.tts_ratio else "asr"
            else:
                task = args.tasks[0]
    ...
    print("Packer ensuite avec : python -m s2s_toolcalling.data.preprocess_sft "
