#!/usr/bin/env python3
"""Smoke-test progressif du plugin lfm2_audio.vllm_plugin (cible : Colab GPU).
...
Usage (Colab) :
    lfm2-smoke
    lfm2-smoke --checkpoint exports/full_omni --engine
...
from vllm_omni.config.stage_config import _PIPELINE_REGISTRY, StagePipelineConfig, register_pipeline  # noqa: F401
...
# Champs de StagePipelineConfig utilisés par lfm2_audio.vllm_plugin/pipeline.py.
...
def check_plugin() -> list[str]:
    ...
    try:
        ...
        if registered:
            ...
            problems.append(_fail("plugin", f"architecture {ARCHITECTURE!r} introuvable dans OmniModelRegistry"))
    except Exception as exc:
...
def check_engine(checkpoint: Path, max_tokens: int, dtype: str) -> list[str]:
    ...
    try:
        ...
        omni = Omni(
            ...
            # le stage 1 (cf. configs/serving/vllm_omni.yaml, mesuré 12/06)
