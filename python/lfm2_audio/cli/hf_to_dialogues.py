...
def rehydrate(repo_id: str, out: str | Path) -> dict[str, int]:
    ...
    for split, raw_split in dd.items():
        ds = raw_split.cast_column("audio", Audio(sampling_rate=16_000))
        ...
        with (out / f"{split}.jsonl").open("w", encoding="utf-8") as f:
            for row in ds:
                ...
                sf.write(str(audio_root / rel), row["audio"]["array"], row["audio"]["sampling_rate"], subtype="PCM_16")
