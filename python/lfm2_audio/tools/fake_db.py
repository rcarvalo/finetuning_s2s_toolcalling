"""Fake DB en mémoire pour la démo ``db_query`` (données de ``configs/sql/schema_en.sql``).
...
ORDERS = [
    {
        "id": o["id"],
        "customer": _CUST[o["customer_id"]],
        "product": _PROD[o["product_id"]],
        "quantity": o["quantity"],
...
_ROUTES: list[tuple[str, list[str], list[dict]]] = [
    (
        "orders",
        ["order", "orders", "invoice", "shipped", "delivered", "cancelled", "open order", "purchase"],
        ORDERS,
    ),
    ("meetings", ["meeting", "schedule", "appointment", "calendar"], MEETINGS),
    (
        "employees",
        ["employee", "employees", "staff", "team", "hire", "hired", "sales team", "engineer"],
        EMPLOYEES,
    ),
