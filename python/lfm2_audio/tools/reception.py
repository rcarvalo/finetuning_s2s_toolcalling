...
class InMemoryReceptionBackend:
    ...
    async def find_appointments(self, visitor_name: str, host_name: str | None, time_hint: str | None) -> list[dict]:
        ...
        results = [
            a for a in self.appointments if _norm(str(a["visitor_name"])) == v or v in _norm(str(a["visitor_name"]))
        ]
        if host_name:
            h = _norm(host_name)
            results = [a for a in results if h in _norm(str(a["host"]))]
        return results

    async def find_employee(self, name: str) -> dict | None:
        n = _norm(name)
        for e in self.employees:
            if n in _norm(str(e["full_name"])):
                return e
        return None
