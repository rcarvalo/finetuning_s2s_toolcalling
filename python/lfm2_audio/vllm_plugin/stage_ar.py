...
from lfm2_audio.vllm_plugin.constants import IM_END_TOKEN_ID
from lfm2_audio.vllm_plugin.depthformer_graph import CudaGraphDepthformer
from lfm2_audio.vllm_plugin.modality import Modality, ModalityConfig, replay
...
class Lfm2AudioARForConditionalGeneration(nn.Module):
    ...
    def _resolve_tool_call_ids(vllm_config, config) -> tuple[int | None, int | None]:
        ...
        if start is None or end is None:
            logger.warning(
                "tool-call lock disabled: <|tool_call_start|>/<|tool_call_end|> not single tokens (start=%s end=%s)",
                start,
                end,
            )
            return None, None
        ...
        **kwargs: Any,
        ...
        if multimodal_embeddings is not None and is_multimodal is not None:
            if isinstance(multimodal_embeddings, (list, tuple)):
                multimodal_embeddings = torch.cat(list(multimodal_embeddings)) if multimodal_embeddings else None
        ...
        if os.environ.get("LFM2_DEBUG_MM"):
            ...
            logger.warning(
                ...
                int(input_ids.numel()),
                n_au,
                (int(is_multimodal.sum()) if is_multimodal is not None else None),
                scattered,
            )
        return embeds
    ...
    def sample(self, logits: torch.Tensor, sampling_metadata):
        ...
        if len(self._step_req_ids) == n_rows:
            ...
            if self._step_req_ids:
                logger.warning(
                    ...
                    n_rows,
                    len(self._step_req_ids),
                    len(self._step_sampling_reqs),
                )
            return None
        ...
        for row in range(n_rows):
            history = sampling_metadata.output_token_ids[row] if row < len(sampling_metadata.output_token_ids) else []
        ...
        if audio_rows:
            ...
            for row, tok in zip(audio_rows, sampled_audio, strict=True):
                sampled[row] = tok
    ...
    def _sample_audio_rows(self, rows: list[int], request_ids: list[str]) -> list[int]:
        ...
        for j, request_id in enumerate(request_ids):
            ...
            sampled.append(
                self.modality_cfg.eoa_placeholder_id if bool(is_eoa[j]) else self.modality_cfg.frame_placeholder_id
            )
    ...
    def load_weights(self, weights: Iterable[tuple[str, torch.Tensor]]) -> set[str]:
        ...
        lfm_weights = [("model." + k[len("lfm.") :], v) for k, v in state.items() if k.startswith("lfm.")]
        lfm_loaded = self.language_model.load_weights(lfm_weights) or set()
        loaded.update(f"language_model.{name}" for name in lfm_loaded)

        enc_state = {k[len("conformer.") :]: v for k, v in state.items() if k.startswith("conformer.")}
        ...
        adapter_state = {k[len("audio_adapter.") :]: v for k, v in state.items() if k.startswith("audio_adapter.")}
        ...
        self.audio_head.load_weights(state)
