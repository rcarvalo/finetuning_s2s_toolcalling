...
def render_python_literal(value: Any) -> str:
    ...
    if isinstance(value, dict):
        items = ", ".join(f"{json.dumps(str(k), ensure_ascii=False)}: {render_python_literal(v)}" for k, v in value.items())
        return "{" + items + "}"
...
def render_tool_response(payload: Any) -> str:
...
def verify_special_tokens(tokenizer: Any) -> dict[str, bool]:
