...
def apply_freeze_policy(model, policy: FreezePolicy) -> dict[str, int]:
    ...
    for name, param in model.named_parameters():
        freeze = False
        if policy.freeze_encoder and name.startswith(ENCODER_PREFIXES):
            freeze = True
        elif policy.freeze_audio_heads and name.startswith(AUDIO_HEAD_PREFIXES):
            freeze = True
        elif policy.freeze_backbone and name.startswith(BACKBONE_PREFIX) and "lora_" not in name:
            freeze = True
