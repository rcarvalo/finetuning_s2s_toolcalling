...
def build_agent_from_config(config: dict):
    ...
    from s2s_toolcalling.tools.reception import InMemoryReceptionBackend, PostgresReceptionBackend, build_reception_registry
    ...
    if dsn:
        ...
        backend = PostgresReceptionBackend(db)
    else:
    ...
    if rag_cfg.get("persist_dir"):
        from s2s_toolcalling.rag.retriever import KnowledgeBaseRetriever
...
def create_app(agent):
    ...
    from s2s_toolcalling.orchestrator.events import AudioChunk, event_to_dict
                ...
                if mtype == "audio":
                    ...
                    while True:
                        ...
                        if isinstance(event, AudioChunk):
                            samples = event.samples.numpy() if hasattr(event.samples, "numpy") else np.asarray(event.samples)
